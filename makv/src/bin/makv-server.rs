...
impl<E: MakvEngine> YakvServer<E> {
    ...
    fn send_response(&mut self, stream: &mut TcpStream, res: Response) -> Result<()> {
        let (_, bytes) = MakvMessage::get_len_payload_bytes(Payload::Response(res))?;
    ...
    fn handle_request(&mut self, mut stream: &mut TcpStream) -> Result<Response> {
        let message = MakvMessage::new(&mut stream, PayloadType::Command)?;
...
fn main() -> Result<()> {
    ...
    if !existing_engines.is_empty() && !existing_engines.contains(&config.engine) {
        return Err(MakvError::Any(anyhow!(
    ...
    match config.engine {
        ...
        Engine::Sled => {
            let store = SledStore::open(current_dir.as_path())?;
            let mut server = YakvServer::new(config, log, store);
            server.start()?;
        }
    };
