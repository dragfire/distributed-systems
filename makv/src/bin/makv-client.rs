...
use makv::{Command, MakvMessage, Payload, PayloadType, Result};
use std::env;
use std::io::Write;
...
fn main() -> Result<()> {
    ...
    stream.write_all(&MakvMessage::get_len_payload_bytes(Payload::Command(cmd))?.1)?;
    stream.flush()?;
    let response = MakvMessage::new(&mut stream, PayloadType::Response)?;
