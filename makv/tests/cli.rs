...
// `makv-client` with no args should exit with a non-zero code.
#[test]
fn client_cli_no_args() {
    let temp_dir = TempDir::new().unwrap();
    let mut cmd = Command::cargo_bin("makv-client").unwrap();
...
// `makv-client -V` should print the version
#[test]
...
// `makv-server -V` should print the version
#[test]
fn server_cli_version() {
    let temp_dir = TempDir::new().unwrap();
    let mut cmd = Command::cargo_bin("makv-server").unwrap();
    cmd.args(&["-V"])
        .current_dir(&temp_dir)
        .assert()
        .stdout(contains(env!("CARGO_PKG_VERSION")));
...
fn cli_log_configuration() {
    ...
    let mut cmd = Command::cargo_bin("makv-server").unwrap();
    let mut child = cmd
        .args(&["--engine", "makv", "--addr", "127.0.0.1:4001"])
        .current_dir(&temp_dir)
    ...
    assert!(content.contains("makv"));
    assert!(content.contains("127.0.0.1:4001"));
}

#[test]
fn cli_wrong_engine() {
    // sled first, makv second
    {
        let temp_dir = TempDir::new().unwrap();
        let mut cmd = Command::cargo_bin("makv-server").unwrap();
        let mut child = cmd
        ...
        let mut cmd = Command::cargo_bin("makv-server").unwrap();
        cmd.args(&["--engine", "makv", "--addr", "127.0.0.1:4003"])
            .current_dir(&temp_dir)
            .assert()
            .failure();
    }

    // makv first, sled second
    {
        let temp_dir = TempDir::new().unwrap();
        let mut cmd = Command::cargo_bin("makv-server").unwrap();
        let mut child = cmd
            .args(&["--engine", "makv", "--addr", "127.0.0.1:4002"])
            .current_dir(&temp_dir)
            .spawn()
            .unwrap();
        thread::sleep(Duration::from_secs(1));
...
fn cli_access_server(engine: &str, addr: &str) {
    ...
    let temp_dir = TempDir::new().unwrap();
    let mut server = Command::cargo_bin("makv-server").unwrap();
...
#[test]
fn cli_access_server_makv_engine() {
    cli_access_server("makv", "127.0.0.1:4004");
}
